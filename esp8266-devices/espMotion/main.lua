...
data.deviceName = 'Motion1' --Replace with your device's name
data.deviceSerial = '5722ff4f38' --Replace with thid device's unique serial number
data.slotActive = 0
	...
	tmr.alarm(0, 3000, tmr.ALARM_AUTO, function()
		...
		if ip == nil and joinCounter < joinMaxAttempts then
			...
			if(referer == 'newNetwork' and returnvalue) then
				...
				--data.pairKey = slot[data.slotActive]['key']
				getToken()
				--startUpdates()
	  ...
	  function(code, response)
	    ...
	    if (code < 0) then
	      ...
	      if (data.jsonResponse['success'] == 'OK') then
	      	...
	      	data.jsonPostUpdate = '{"action":"logMotion","serial":"'..data.deviceSerial..'","token":"'..data.token..'", "motion":"'..data.motion..'"}'
	      	startUpdates()
	      else
	      	blink('fast')
	      	node.restart()
...
function startUpdates()
	getUpdate()
	tmr.alarm(3,5000, tmr.ALARM_AUTO, function() getUpdate() end)
end
...
function serverModeStart()
	...
	srv:listen(80,function(conn)
  		conn:on("receive",function(conn,payload)
    		--get post variables here and parse
