...
data.deviceName = 'TempHum2' --Replace with your device's name
data.deviceSerial = 'e32842f282' --Replace with thid device's unique serial number
data.slotActive = 0
...
gpio.mode(dhtSensor, gpio.INPUT)
...
function updateTemperature()
	...
	local retries = 0
	local flag = false
	while(retries < 5 and flag == false) do
		retries = retries+1
		status, temp, humi, temp_dec, humi_dec = dht.read11(dhtSensor)
		if status == dht.OK then
			data.temp = math.floor(temp)
			data.hum = math.floor(humi)
			flag = true
		end
	end
	if (not flag) then
	  ...
	  function(code, response)
	    ...
	    if (code < 0) then
	      ...
	      if (data.jsonResponse['success'] == 'OK') then
	      	...
	      	data.jsonPostUpdate = '{"action":"logTemphum","serial":"'..data.deviceSerial..'","token":"'..data.token..'", "temp":"'..data.temp..'", "hum":"'..data.hum..'"}'
	      	startUpdates()
	      else
	      	blink('fast')
	      	node.restart()
...
function startUpdates()
	getUpdate()
	tmr.alarm(3,20000, tmr.ALARM_AUTO, function() getUpdate() end)
end
